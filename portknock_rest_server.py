...
class Portknock_Server(ControllerBase):
    ...
    def get_portknocking_info(self):
        ...
        servers = {}
        servers['ipv4'] = self.port_knocking.active_keys
        servers['ipv6'] = self.port_knocking.active_keys_v6
        rv['servers'] = servers
        rv['auth_port'] = self.port_knocking.auth_port
        rv['authenticating_hosts'] = self.port_knocking.authing_hosts
    ...
    def remove_authed_host(self, host, dst_host):
        """ Removes IP address from authenticated hosts """
        if self.port_knocking.remove_host_access(host, dst_host):
            return '204 OK'
